...
phraseology_dict = {
  "Бить баклуши": {
    ...
    "Происхождение": "Вероятно, связано с развлечением, которое называлось 'бить по баклушам' - бить палкой по острым стеблям мака для того, чтобы сбить семена. Выражение впервые употребил в речи Ломоносов.",
  ...
  "Водить за нос": {
    ...
    "Происхождение": "Предположительно, связано с образом дрессировщика, который водит медведя на поводке, используя кольцо, продетое в нос животного.",
    "Примеры использования": [
  ...
  "Гнуть свою линию": {
    ...
    "Происхождение": "Вероятно, связано с образом упрямого человека, который гнет гвоздь, несмотря на трудности.",
    "Примеры использования": [
  ...
  "Как белка в колесе": {
    "Описание": "Быть в постоянной суете, заниматься множеством дел одновременно.",
...
# Функция для показа вопросов и получения ответов

def show_question(question_data, question_number):
    st.write(f"**Вопрос {question_number}:** {question_data['Вопрос']}")
    
    # Перемешиваем ответы перед отображением
    random.shuffle(question_data['Ответы'])
    
    selected_option = st.radio(f"Выберите правильный ответ на вопрос {question_number}:", question_data["Ответы"])
...
def main():
    ...
    # Инициализация переменной для подсчёта правильных ответов
    score = 0

    # Проход по всем вопросам
    for i, question_data in enumerate(questions, start=1):
        if st.button(f"Показать вопрос {i}"):
            selected_option = show_question(question_data, i)
